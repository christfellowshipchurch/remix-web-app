...
import { VolunteerTripCard } from "../components/cards/volunteer-trip-card.component";

export function VolunteerGlobe() {
  const { volunteerTrips } = useLoaderData();

  // Create an array of the first trip from each country
  const topTrips: Trip[] = Object.values(volunteerTrips as Record<string, Trip[]>)
    .map((trips) => trips[0])
    .filter(Boolean);
  ...
  const isNew = (trip: Trip) => {
    ...
    const tripsFromThatCountry = volunteerTrips[tripCountry] || [];
  ...
  return (
    <section id="globe" className="w-full bg-white py-28 content-padding">
      <div className="max-w-screen-content mx-auto">
        <SectionTitle sectionTitle="volunteer" />
        ...
        <div className="flex flex-col-reverse lg:flex-row items-center justify-center gap-12">
          <div className="flex flex-col gap-4">
            {topTrips.map((trip) => (
              <VolunteerTripCard key={trip.id} trip={trip} isNew={isNew(trip)} />
            ))}
          </div>
