...
function refinementChipDisplayLabel(attribute: string, value: string): string {
  if (attribute === EVENT_FACET_CATEGORIES) {
    return startCase(value);
  }
  if (attribute === EVENT_FACET_LOCATIONS) {
    return value;
  }
  return value;
}

export function EventsUrlActiveFilters({
  urlState,
  applyUrlState,
  onClearAllToUrl,
}: {
  ...
  const refinementChips = useMemo((): RefinementChip[] => {
    ...
    for (const [attribute, values] of Object.entries(rl)) {
