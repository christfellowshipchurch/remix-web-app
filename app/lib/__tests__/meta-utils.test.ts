...
describe("createMeta", () => {
  ...
  it("includes description in meta and OG tags", () => {
    ...
    expect((descMeta as { content?: string })?.content).toBe("Test description");
  ...
  it("uses default image when none provided", () => {
    ...
    expect((ogImage as { content?: string })?.content).toContain(DEFAULT_META_IMAGE);
  });
  ...
  it("uses custom keywords when provided", () => {
    ...
    const keywords = meta.find((m) => "name" in m && m.name === "keywords");
    expect((keywords as { content?: string })?.content).toBe("custom, keywords");
  });
  ...
  it("uses default generator", () => {
    ...
    expect((generator as { content?: string })?.content).toBe(DEFAULT_GENERATOR);
  });
  ...
  it("includes og:type as website", () => {
    const meta = createMeta({ title: "Test", description: "Test" });
    const ogType = meta.find((m) => "property" in m && m.property === "og:type");
