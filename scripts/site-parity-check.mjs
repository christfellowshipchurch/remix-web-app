...
async function fetchText(url, { timeoutMs = 15000 } = {}) {
  ...
  try {
    ...
    return { ok: false, status: 0, contentType: "", body: "", error: String(err) };
  } finally {
    clearTimeout(t);
  }
}
...
async function collectSitemapUrls(sitemapUrl, seen = new Set()) {
  ...
  const res = await fetchText(sitemapUrl, { timeoutMs: CONFIG.crawl.timeoutMs });
  if (!res.ok || !res.body) {
  ...
  if (doc.urlset?.url) {
    return dedupe(asArray(doc.urlset.url).map((u) => u.loc).filter(Boolean));
  }
...
// ---------------------------------------------------------------------------
const ASSET_RE = /\.(png|jpe?g|gif|svg|webp|ico|css|js|mjs|map|json|xml|txt|pdf|zip|woff2?|ttf|eot|mp4|webm|mp3|wav)$/i;
...
function loadGsc(path) {
  ...
  const urlCol = header.findIndex((h) => h.includes("page") || h.includes("url") || h.includes("address"));
...
function toCsv(rows) {
  ...
  const headers = ["old_path", "old_url", "status", "suggested_new_path", "match_reason", "in_gsc", "gsc_clicks"];
...
async function main() {
  ...
  if (gsc.size) {
    ...
    console.log("   (no GSC export provided — pass GSC_CSV=./export.csv to include traffic data)");
  }
  ...
  console.log(`   ${crawled.size} URLs from crawl; ${newPathsSet.size} new paths total`);
  ...
  // Sort: missing (with traffic first) > likely-moved > present > ignore
  const order = { missing: 0, "likely-moved": 1, present: 2, ignore: 3 };
  rows.sort((a, b) =>
  ...
  console.log("\n=== SUMMARY ===");
  for (const k of ["missing", "likely-moved", "present", "ignore"]) {
    if (counts[k]) console.log(`  ${k.padEnd(13)} ${counts[k]}`);
  }
  const missingWithTraffic = rows.filter((r) => r.status === "missing" && r.in_gsc).length;
...
export {
  collectSitemapUrls, crawl, normalizePath, loadGsc, parseCsv,
  findLikelyMove, levenshtein, isIgnored, toCsv,
};
